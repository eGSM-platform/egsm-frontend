...
export class EnginesComponent {
  ...
  onTest(){
    ...
    //console.log(this.bpmnDiagrams.map(p => p.model_id).join(', '))
    //this.bpmnDiagrams.forEach(element =>{
    //  console.log(element.updateModelXml(DIAGRAM))
    //  //element.updateModelXml(DIAGRAM)
    //})
    //this.bpmnDiagram.updateModelXml(DIAGRAM)
  }
  ...
  onSearch(instance_id: any) {
    ...
    //let truck = this.bpmnDiagrams.forEach(element=>{element.updateModelXml(DIAGRAM)})
